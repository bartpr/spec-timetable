...
void Genotype::Evaluation(Data &d)
{
	...
	if (QueryPerformanceFrequency(&lpFrequency) != 0)
	{
		...
		for(int i = 0; i < 1000000; i++)
		{
			...
			mark += collisionsInClass(d); //kolizje w jednej klasie
...
double Genotype::termsCollision()
{
	...
	//przechodzimy przez wszystkie geny, czyli wszystkie wystepuj¹ce kombinacje terminow u sali
...
double Genotype::evalSubject(const Data &d, Data::Node *p)//p to dana grupa
{
	...
	for( int i= 0; i< p->numberOfLessons; i++ )
	{
		...
		else if(d.tab[p->lessons[i]].subject != d.tab[p->lessons[i + 1]].subject)//jezeli nastepny przedmiot w tablicy jest inny od obecnego
				tmpPenalty += evalSubject_(tab);//ocena f. pomocnicza
	}
...
template <class T>
void selection_sort(T* tab, int n)
{
	for(int i = 0; i < n; i++)
	{
		int k = i;
...
double Genotype::evalSubject_(short int *tab)//f. pomocnicza do evalSubject, odnosi sie juz do konkretnego przedmiotu
{
	...
	for(int i = 0; i < numberOfDays; i++)//liczenie ilosci godzin
		hours_total += days[ i ];
	selection_sort(days, numberOfDays);//posortowanie tablicy z liczbami godzin w danym dniu
	//Tworzenie idealnego rozdzialu blokow
	...
	for(int i = 0; i < numberOfDays; i++)
	{
		if(days[i] == 0 && ideal[i] != 0) //brak dnia
			...
			tmp += pow(static_cast<double>(days[i]-ideal[i]), 2) * hour_add_cost * sqrt(static_cast<double>(ideal[i]/days[i]));
...
double Genotype::collisionsInClass(Data &d, Data::Node *p, Data::Node *q) //p, q - wezly, ktorymi sprawdzamy kolizje
{
	const double collisionPenalty = 1; //wspolczynnik kary za 1 kolizje w klasie
	double tmpPenalty = 0; //aktualna kara
	if(p == 0 && q == 0) //pierwsze wywolanie
	{
		//sprawdzamy kolizje korzenia z kazda podgrupa
		...
		cleanVectors(d.k); //wyzerowanie wektora checked w kazdym wezle
		return tmpPenalty;
	}
	...
	else
	{
		...
		for(int i = 0; i < q->checked.size(); i++)
			if(q->checked[i] == q->id) //czy byly sprawdzane kolizje wewnetrzne w grupie q
				inGroupQ = true;
		if(!betweenGroups) //jeœli nie byly sprawdzane kolizje miedzy p i q
		{
			bool *tab = new bool[d.numberOfTerms]; //tablica pomocnicza o rozmiarze wszystkich mozliwych terminow
			for(int i = 0; i < d.numberOfTerms; i++) //zerowanie tablicy pomocniczej
				tab[i] = false;
			for(int i = 0; i < p->numberOfLessons; i++) 
				//jesli ten termin wystepuje pierwszy raz to oznaczamy go w tablicy pomocniczej
				if(!tab[genes[p->lessons[i]]->term])
					tab[genes[p->lessons[i]]->term] = true;
				else if(!inGroupP) //jesli nie byly sprawdzane kolizje w wezle p to liczymy kolizje
				{
					tmpPenalty -= collisionPenalty;
					if(!penalty) //ustawiamy flage kary, jesli nie jest ustawiona
						penalty = true; 
				}
			for(int i = 0; i < q->numberOfLessons; i++)
				if(tab[genes[q->lessons[i]]->term]) //termin jest oznaczony w tablicy pomocniczej = kolizja
				{
        			tmpPenalty -= collisionPenalty;
					if(!penalty)
						penalty = true;
				}
				else if(!inGroupQ) //jesli nie byly sprawdzane kolizje w wezle q to oznaczamy termin w tablicy pomocniczej
					tab[genes[q->lessons[i]]->term] = true;
			//dodajemy nr wezla q do sprawdzonych wezlow w p; graf jest skierowany, wiêc nie trzeba robic tego samego z q
			p->checked.push_back(q->id);
			//dodajemy wezel do sprawdzonych wezlow w samym sobie, by potem sprawdzac, czy w byly w nim sprawdzane kolizje 
			if(!inGroupP)
				p->checked.push_back(p->id);
			if(!inGroupQ)
				q->checked.push_back(q->id);
			delete[] tab; //usuniecie tablicy pomocniczej
		}
		//sprawdzamy blokowanie przedmiotow w wezlach, jesli wczesniej nie bylo sprawdzone
		if(!inGroupP)
...
{
	//funkcja zeruje wektory w ka¿dym wêŸle
	if(p->checked.empty())
		p->checked.clear();
		...
		for(int j = 0; j < numberOfDays; j++)//w ka¿dym dniu tygodnia
		{
			...
			for (int k=0;k<numberOfTerms/numberOfDays;k++)//przejscie przez kolejne terminy w dniu
			{
				if(tab[i][j*(numberOfTerms/numberOfDays)+k])//jesli w tym terminie odbywaja sie zajecia
				{
					...
					// dzieki tym danym mozna bardzo latwo sprawdzac preferencje nauczycieli, nalezaloby to jednak rowniez zaimplementowac
